$.getJSON('/api/tasks/list', function(json, textStatus) {
	$.each(json.tasks, function(index, val) {
		...
		$(cat).append('<div class="task ' + pclass + ' row-fluid"><div class="desc span8"><div class="title">' + val.title + '</div><div>' + val.note + '</div></div><div class="time span4"><div class="date">' + due + '</div><div>created ' + val.created_ago + '</div></div>');
	});
});